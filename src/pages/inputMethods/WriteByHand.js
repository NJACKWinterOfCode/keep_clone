...
export default class WriteByHand extends Component {
    ...
    findxy(res, e) {
        ...
        let scaleWidth = this.canvas.current.clientWidth,
            scaleHeight = this.canvas.current.clientHeight;
        let actualWidth = this.canvas.current.width,
            actualHeight = this.canvas.current.height;
        if (res === 'down') {
            this.setState({
    ...
    render() {
        return ( <
            ...
            alt = "Write By Hand" / > < /button>  <
            button onClick = {
                this.clearcanvas1
            }
            className = "btn" > Reset < /button> </div > < canvas id = "can"
            ref = {
                this.canvas
            }
            className = "canvas"
            ...
            /> </div > < div className = "eraser" >
            <
            div className = "Eraser" > Eraser < /div> <div className = "EraserIcon"
            id = "white"
            onClick = {
                this.color
            } > < /div> </div > < /div> <button onClick = {this.addNote}
            type = "submit"
            className = "btn btn-block " > Add Note < /button> </div > );
    }
}